...
// kodeoevelse). Haiku (ca. 1/3 af prisen) kun til den mest mekaniske opgave
// (omformulering af ét enkelt spoergsmaal), hvor Sonnet-kvalitet ikke er noedvendig.
...
export default async function handler(req, res) {
  ...
  try {
        ...
        // Trim prompt. 'summary'/'explain'/'fag-summary' faar et lavere loft end
        ...
        // parse-fejl). Opgraderet fra Haiku til Sonnet for mere gennemtænkte,
        // varierede opgaver — kaldet er kort/sjældent, så merudgiften er lille.
        if (mode === 'code-exercise') {
            ...
            const text = await callClaude(system, cachedContent(prompt, instruction), 1536, MODEL_SONNET);
                  const clean = text.replace(/```json|```/g, '').trim();
                  try {
        ...
        // maxTokens saenket fra 4096 -> 2800 for at holde svartiden sikkert under
        // Vercels 60-sekunders tidsgraense (se kommentar ved promptCap ovenfor).
        if (mode === 'fag-summary') {
                  const system = 'Du laver eksamensopsummeringer på dansk. Brug markdown: # overskrifter, ## underoverskrifter, **fed** for nøglebegreber, - for punktlister. Vær grundig men præcis, og hold svaret fokuseret frem for udtømmende, så det kan genereres hurtigt og pålideligt.';
                  const text = await callClaude(system, prompt, 2800);
